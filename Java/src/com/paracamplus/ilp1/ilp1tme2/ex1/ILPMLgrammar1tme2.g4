...
/*
 func returns [com.paracamplus.ilp1.interfaces.IAST node]
    //déclaration de function
    
    : 'function' f+=IDENT '(' args+=IDENT? (',' args2+=IDENT)* ')' value+=expr #Function
    ;
*/
...
// while 
    | 'while' condition=expr 'do' exprs+=expr # While
    
// function
    | 'function' name=IDENT '(' vars+=IDENT? (',' vars+=IDENT)* ')' exprs+=expr # Function
