...
typedef struct ILP_Object {
     ...
     union {
          ...
          struct asVector {
               int size;
               struct ILP_Object* asILP_Object[1]; 
          } asVector; /*nouveau variant vecteur*/
     }                  _content;
} *ILP_Object;
...
/** Ajout pour Vector **/
/* verifier si ces ajouts sont utiles  */

#define ILP_Vector2ILP(size,o) \
    ILP_make_vector(size,o)
    
#define ILP_AllocateVector(size) \
    ILP_malloc(sizeof(struct ILP_Object) \
        + (sizeof(ILP_Object) * size), &ILP_object_Vector_class)
...
/* modification */
/*
* ici on declare une variable extern, de type struct ILP_Class et cette variable 
* s'appelle ILP_object_Vector_class
*/
...
/*modification*/
extern ILP_Object ILP_make_vector (ILP_Object size, ILP_Object o);
extern ILP_Object ILP_vector_get (ILP_Object o, ILP_Object position);
extern ILP_Object ILP_vector_length (ILP_Object o);
