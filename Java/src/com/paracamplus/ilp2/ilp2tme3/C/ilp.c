...
ILP_Object
ILP_make_vector (ILP_Object size, ILP_Object o)
{
    if (ILP_isInteger(size)){	
    	int s = size->_content.asInteger;
    	ILP_Object result = ILP_AllocateVector(s);
    	result->_content.asVector.size = s;
    	int i ;
    	for (i=0;i<s;i++){
        	result->_content.asVector.asILP_Object[i] = o;
    	}
    	return result;
    }else{
    	return ILP_domain_error("First arguemt must be an expression given a ILP_Object asinteger",size);
    }

}

ILP_Object
ILP_vector_get (ILP_Object o, ILP_Object position)
{   
	int p=0;
    if (ILP_isVector(o)){
    	if (ILP_isInteger(position)){	
    		p = position->_content.asInteger;
        	if (p >= o->_content.asVector.size){
           		return ILP_error("Position asked must be smaller than size");
      		}
      	}else{
      		return ILP_domain_error("second argument must be an expression given a ILP_Object asinteger",position);
      	}
        return o->_content.asVector.asILP_Object[p];
    }else{
...
ILP_Object
ILP_vector_length (ILP_Object o){
    if (ILP_isVector(o)){
        ILP_Object result = ILP_AllocateInteger();
        result->_content.asInteger = (o->_content.asVector.size);
        return result;
    }else{
        return ILP_domain_error("Not a vector", o);
       }
...
ILP_sinus (ILP_Object self)
{
	...
	if (ILP_isInteger(self)){
		ILP_Object result = ILP_AllocateFloat();
		result->_content.asFloat = sin((float)self->_content.asInteger);
		return result;
	}
	else{
		return ILP_domain_error("Not a number.",self);
	}
